...
quickApplyOnly = input("Would you like to only apply to quick apply jobs? (y/n) ")
...
try:
  pages = driver.find_elements_by_css_selector('.artdeco-pagination__pages li')
...
for i in range(len(jobs)):
  ...
  try:
    ...
    if submit_button.get_attribute("aria-label") == "Continue to next step":  
      ...
      print("Applied")
      submit_button.click()  
      driver.find_element_by_css_selector('[aria-labelledby="post-apply-modal"] button').click()
